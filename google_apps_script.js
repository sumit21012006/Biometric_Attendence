 ...
 * 7. Configure:
 *    - Select type: Web app
 *    - Description: Biometric Attendance Webhook with School Name
...
function doPost(e) {
  ...
  try {
    ...
    // Auto-create spreadsheet columns headers if sheet is empty (now 12 columns)
    if (sheet.getLastRow() === 0) {
      ...
      sheet.getRange(1, 1, 1, 12).setBackground("#e2e8f0");
    ...
    if (lastRow >= 2) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (cellDateObj) {
          ...
          var cellUid = String(row[11] || row[2] || "").trim(); // Index 11 is UID
          var cellType = String(row[7]).toUpperCase().trim(); // Index 7 is Type
...
function autoCheckOutDaily() {
  ...
  // Get all data columns (Date, Time, Employee ID, Name, Email, Designation, School Name, Type, Device ID, Distance, Verified, UID)
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    if (rowDateStr === todayStr) {
      var uid = String(row[11] || row[2]).trim(); // Column 12 (index 11) is UID
      var empId = String(row[2]).trim();
      var type = String(row[7]).toUpperCase().trim(); // Column 8 (index 7) is Type
  ...
  for (var uid in checkIns) {
    if (!checkOuts[uid]) {
      ...
      var targetRowRange = sheet.getRange(appendedRowIndex, 1, 1, 12);
...
function generateMonthlyReport(targetMonth, targetYear) {
  ...
  // Read all logs (Date, Time, Employee ID, Name, Email, Designation, School Name, Type, Device ID, Distance, Verified, UID)
  var rawRange = rawSheet.getRange(2, 1, lastRow - 1, 12);
  var rawValues = rawRange.getDisplayValues();
  ...
  for (var i = 0; i < rawValues.length; i++) {
    ...
    if (!employees[empId]) {
      ...
      if (schoolName && schoolName !== "N/A") employees[empId].schoolName = schoolName;
    }
  }
  ...
  for (var i = 0; i < rawValues.length; i++) {
    ...
    var timeCell = row[1];
    var empId = String(row[2]).trim();
    var type = String(row[7]).toUpperCase().trim(); // Index 7 is Type
  ...
  // Create beautiful merged double headers (including School Name)
  var headersRow1 = ["Employee ID", "Name", "Designation", "School Name"];
  var headersRow2 = ["", "", "", ""];
  ...
  // Merge Day headers in Row 1 horizontally across 3 columns (shifted startCol to 5)
  for (var d = 1; d <= numDaysInMonth; d++) {
    var startCol = 5 + (d - 1) * 3;
    reportSheet.getRange(1, startCol, 1, 3).merge();
  }

  // Merge Employee ID, Name, Designation, and School Name headers vertically
  reportSheet.getRange("A1:A2").merge();
  reportSheet.getRange("B1:B2").merge();
  reportSheet.getRange("C1:C2").merge();
  reportSheet.getRange("D1:D2").merge();
  ...
  var totalCols = 4 + numDaysInMonth * 3;
  ...
  if (totalRows > 2) {
    reportSheet.getRange(3, 1, totalRows - 2, 4).setHorizontalAlignment("left"); // Info columns (A-D)
  ...
  reportSheet.setFrozenColumns(4); // Freeze Employee ID, Name, Designation, and School Name columns
